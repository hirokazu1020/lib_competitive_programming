...
public:
	...
	int depth(int u)const{//rootÇÃê[Ç≥Ç™1
		return _depth[u];
	}
};
...
class BIT{//binary indexed tree, Fenwick treeÇ∆Ç‡
	std::vector<int> bit;
public:
	...
	void add(int i,int x){//iî‘ñ⁄Ç…xâ¡Ç¶ÇÈ
		i++;//BITìYÇ¶éöÇÕ1Å`ÇéÇæÇ©ÇÁ
		while(i<(int)bit.size()){
			bit[i]+=x;
			i+= i&-i;
		}
	}
	int sum(int a)const{//[0,a]ÇÃòa
		a++;
		int res=0;
		while(0<a){
			res+=bit[a];
	...
	int sum(int a,int b)const{//[a,b]ÇÃçáåv
...
public:
	...
	long long query(int u,int v)const{//u->vÇÃí∏ì_ÇÃòa
		int w=lca.query(u,v);
		pair<int,int> a,b,c;
		a=ver[u];
		b=ver[v];
