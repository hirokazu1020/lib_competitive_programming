...
class RBSTArray{
	...
	static Node *merge(Node *a,Node *b){
		if(!a)return b;
		if(!b)return a;
		if( a->height*0xffffff > (a->height+b->height)*(xor128()&0xffffff)  ){
	...
	Node *root;
public:
	RBSTArray(Node* a):root(a){}
	RBSTArray():root(0){}
	~RBSTArray(){if(root)delete root;}
	void insert(int k,int v){
		std::pair<Node*,Node*> p=split(root,k);
		root=merge(p.first,merge(new Node(v),p.second));
	}
	void merge(RBSTArray &a){//*thisとaを合併
	...
	void split(std::pair<RBSTArray,RBSTArray > &res, int k){//[0,k),[k..)に分割
